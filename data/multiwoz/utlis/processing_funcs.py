...
def split_bs_text_by_domain(bs_text, bsdx_text):
    ...
    for idx in range(len(domain_list)):
        ...
        if idx == len(domain_list)-1: # last domain
            bs_text_snippet = f'{curr_domain} {bs_text.split(curr_domain)[1].strip()}'
            bsdx_text_snippet = f'{curr_domain} {bsdx_text.split(curr_domain)[1].strip()}'
        else:
            next_domain = domain_list[idx+1]
            bs_text_snippet = f'{curr_domain} {bs_text.split(curr_domain)[1].split(next_domain)[0]}'
            bsdx_text_snippet = f'{curr_domain} {bsdx_text.split(curr_domain)[1].split(next_domain)[0]}'
...
def parse_bs_bsdx(bs_text, bsdx_text):
    ...
    for idx in range(token_num):
        ...
        if idx == token_num - 1:
            ...
            curr_value = bs_text.split(f' {curr_slot} ')[-1].strip()
        else:
            next_slot = bs_name_list[idx+1]
            curr_value = (
                bs_text.split(f' {curr_slot} ')[1]
                .split(f' {next_slot}')[0]
                .strip()
            )
    ...
    for curr_slot in bs_name_list:
        if curr_slot.startswith('[') and curr_slot.endswith(']'):
            res_bs_text += f'{curr_slot} '
            res_bsdx_text += f'{curr_slot} '
        else:
            res_bs_text += f'{curr_slot} = {map_dict[curr_slot]} , '
            res_bsdx_text += f'{curr_slot} , '
...
def overall_parsing_bs_bsdx(bs_text, bsdx_text):
    ...
    for idx in range(len(bs_text_list)):
        ...
        res_bs_text += f'{one_bs_text} '
        res_bsdx_text += f'{one_bsdx_text} '
    res_bs_text = f'<sos_b> {res_bs_text.strip()} <eos_b>'
    res_bsdx_text = f'<sos_b> {res_bsdx_text.strip()} <eos_b>'
...
def parse_action_text(text):
    ...
    for idx in range(token_num):
        ...
        if curr_token.startswith('[') and curr_token.endswith(']'):
            res_text += f'{curr_token} '
        elif idx == token_num - 1:
            res_text += curr_token
        else:
...
def transform_id_dict(in_dict, data):
    return {
        ...
        else data.tokenized_decode(in_dict[key])
        for key in in_dict
    }
