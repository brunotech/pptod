...
def tokenize_text(tokenizer, text, mode):
    if mode == 'bs':
        text = f'<sos_b> {text} <eos_b>'
    elif mode == 'da':
        text = f'<sos_a> {text} <eos_a>'
    elif mode == 'nlg':
        text = f'<sos_r> {text} <eos_r>'
    elif mode == 'usr':
...
def process_file(path_prefix, file_name, tokenizer, output_path_prefix):
    ...
    save_file = f'{output_path_prefix}/{file_name}'
...
if __name__ == '__main__':
    ...
    for dataset_name in dataset_folder_name_list:
        ...
        print(f'{dataset_name} Dataset Tokenization Finished!')
